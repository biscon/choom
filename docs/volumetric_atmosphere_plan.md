...
| 4 | Add the OpenGL 3.3 froxel atlas and clustered light lists | Completed | 2026-08-17 |
| 5 | Add reconstruction, temporal stability, and supported volumetric shadows | Partial | - |
...
### 2026-08-17 — Slice 4 — Completed

- Summary: Replaced Unified's fixed whole-view light march with persistent OpenGL 3.3 RGBA16F medium/light froxel atlases, RGBA32F light/volume records, RGBA8 clustered index lists, a low-resolution integrated target, and separate medium injection, light injection, front-to-back column integration, and HDR composite passes. Added exact quality/aspect sizing, near-square atlas mapping, logarithmic slices, GL texture-limit quality fallback, deterministic 254-view/16-per-cluster selection for lights and compiled local volumes, runtime point-light inclusion, overflow/resource diagnostics, and a bounded scroll area with condensed rows in the 3D preview Atmosphere tab. Unified is now the development default while the Legacy toggle remains available.
- Decisions/deviations folded back into plan: Each coarse cluster band owns exactly eight logarithmic froxel slices, so the fixed Low/Medium/High contracts map to 4/6/8 bands. Light retention uses projected coverage times animated effective intensity and authored scattering, with source kind/stable ID ties; volume retention uses camera-to-AABB distance with stable ID ties. Staging vectors resize only when grid quality/size changes and are filled in place per frame. Shader/resource failure remains latched where applicable, emits one warning, and leaves the analytic fog path intact. Slice 5 temporal history, jitter, reconstruction changes, and shadow sampling were not started; existing selected shadow-slot metadata is only preserved in the light record.
- Files/modules materially affected: Focused volumetric contracts, clustered-list builder, Unified renderer, dynamic-light read access, atmosphere diagnostics, and `SectorMeshRenderer` integration under `sources/sector_demo/renderer`; the existing UI scroll-area API and 3D preview overlay/layout/state; focused diagnostics, cluster, shader-policy, and UI-layout tests; CMake test linkage; this living plan.
- Automated verification: Passed `cmake --build cmake-build-debug -j2`; passed `ctest --test-dir cmake-build-debug --output-on-failure` (28/28); passed focused atmosphere diagnostics and topology/lightmap tests after the final coverage additions; passed `git diff --check`; `git diff --stat` and `git status --short` reviewed.
- GPU measurements: Not captured (user-owned GUI run); Slice 4 has no new quantitative retirement gate beyond the already accepted Slice 2 comparison.
- Manual verification: The user's first Slice 4 GUI launch exposed NVIDIA GLSL 3.30 compile errors in both injection shaders because `packed` was used as an identifier. The reserved identifier was renamed in both clustered-list decoders and covered by the shader-policy regression test; post-fix visual confirmation remains user-owned.
- Cache invalidation behavior: Unchanged. This slice adds renderer-derived per-view cluster data and nonserialized diagnostics UI only; it adds no topology or visible cached 2D editor-state mutation path.
- Lightmap source-hash behavior: Unchanged. Froxel resources, cluster lists, effective quality fallback, renderer diagnostics, and backend selection are visual/runtime-only; no serialized lightmap input, source-hash rule, or bake artifact changed.
- Collision/sector lookup/physics behavior: Unchanged. Projection and clustering read the visual camera and portal visibility but do not feed collision, sector lookup, camera motion, picking, or physics.
- Legacy coexistence/removal state: Legacy local fog, haze, dust ordering, schema, and the temporary diagnostics selector remain intact for comparison. Unified is the initial development backend; no legacy removal or map/hub migration was performed.
- Remaining follow-up within this plan: Slice 5 may add temporal reconstruction/history and supported spotlight shadow sampling when explicitly requested. Do not start Slice 5, migrate hub maps, or retire Legacy as part of Slice 4 completion.

### 2026-08-17 — Slice 5 — Partial

- Summary: Added bilateral depth-aware reconstruction at every quality and Medium/High temporal resolution with fixed Halton jitter sequences, previous camera matrices and atmosphere depth, offscreen/disocclusion rejection, current-neighborhood clamping, finite HDR sanitization, and responsive blending. Added complete lifecycle/content/camera history invalidation with reset diagnostics; reused the two existing dynamic-spotlight shadow owners, matrices, maps, bias, strength, and softness for bounded volumetric PCF; and added nonserialized Composite/Froxels/History Weight views plus Freeze History controls to the Atmosphere diagnostics.
- Decisions/deviations folded back into plan: Low remains current-frame-only with bilateral reconstruction. Medium uses an 8-frame jitter period with 0.20 base/0.65 responsive current-frame weights; High uses 16 frames with 0.10/0.50. Temporal resources are persistent RGBA16F color plus sampleable depth ping-pong targets and resize only with effective target/quality changes. Static lights and point lights remain unshadowed; only an already selected dynamic spotlight slot 0 or 1 may sample a map, and unavailable maps sanitize the clustered slot to unshadowed. A stable fingerprint of compiled local fog and atmosphere-light inputs supplements renderer/probe source revisions so direct static-source changes cannot retain stale history. No new serialized knobs, shadow maps, cubemaps, map migration, or Slice 6 work was added.
- Files/modules materially affected: Focused volumetric contracts/math/renderer, atmosphere diagnostics, and `SectorMeshRenderer` integration under `sources/sector_demo/renderer`; the existing sector-editor 3D preview Atmosphere overlay/layout; focused volumetric math, clustered shadow ownership, shader-policy, diagnostics, and UI-layout tests; CMake test linkage; this living plan.
- Automated verification: Passed `cmake --build cmake-build-debug -j2`; passed `ctest --test-dir cmake-build-debug --output-on-failure` (28/28); all five embedded GLSL 330 fragment shaders passed offline `glslangValidator` compilation; passed `git diff --check`; `git diff --stat` and `git status --short` reviewed. CPU tests cover fixed temporal policies/jitter, reprojection and jitter compensation, offscreen rejection, depth disocclusion, bilateral rejection, finite neighborhood clamp, frame-state reset causes, and existing two-slot dynamic-spot shadow ownership. Existing staging-reuse tests remain passing; the steady path adds no resource resize/allocation or timer-query wait.
- GPU measurements: Not captured (user-owned GUI run); Slice 5 adds no new quantitative gate before Slice 6.
- Manual verification: Not performed (user-owned). Slice remains Partial pending the specified slow walk, sprint, rotation, doorway disocclusion, animated/flickering lights, thin foreground geometry, and camera-teleport checks for ghosting, halos, flicker, and banding.
- Cache invalidation behavior: No topology or visible cached 2D editor-state mutation path was added. Temporal invalidation is renderer-local derived-state invalidation and does not alter `MarkTopologyDocumentEdited()` or the topology render cache.
- Lightmap source-hash behavior: Unchanged. Temporal history, jitter, bilateral reconstruction, spotlight shadow sampling, and debug controls are visual/runtime-only; no serialized lightmap input, `ComputeSectorLightmapSourceHash()` rule, or bake artifact changed.
- Collision/sector lookup/physics behavior: Unchanged. Jitter and previous visual-camera matrices exist only inside atmosphere shaders/renderer state and do not feed the gameplay camera, collision, sector lookup, portal traversal, physics, picking, or lightmap sampling positions.
- Legacy coexistence/removal state: Legacy local fog, haze, dust ordering, schema, and the temporary Legacy/Unified selector remain intact. Unified remains the development default; no legacy removal or hub-map migration was performed.
- Remaining follow-up within this plan: The user performs and reports the Slice 5 visual checklist. If accepted, append a documentation-only Slice 5 completion entry and mark it Completed without beginning Slice 6. Any observed ghosting/halo/flicker/banding issue should be fixed within Slice 5 before migration work.

### 2026-08-17 — Slice 5 — Partial

- Summary: Corrected two diagnostic defects found during the user's first Slice 5 visual check. The Froxels view now composites the nearest-sampled, unfiltered current-frame integrated atmosphere over the scene, exposing low-resolution cells before temporal resolve/bilateral reconstruction instead of stretching raw packed atlas extinction across the screen. Composite, Froxels, and History Weight are now three explicit buttons. Atmosphere diagnostic rows now use the scroll-aware text path, so local coordinates transform into the viewport instead of clipping the leading rows above it.
- Decisions/deviations folded back into plan: Froxels is defined as the actual current integrated froxel-column result before reconstruction, not a packed-atlas texture viewer; the latter produced an opaque milky display with no actionable acceptance signal. A scrollbar appears only when measured diagnostic content exceeds the viewport; at 1080p all correctly positioned rows may fit without one.
- Files/modules materially affected: `SectorVolumetricAtmosphereRenderer` composite/debug selection; the sector-editor preview Atmosphere overlay/layout; focused shader-policy and UI-layout tests; this living plan.
- Automated verification: Passed `cmake --build cmake-build-debug -j2`; passed `ctest --test-dir cmake-build-debug --output-on-failure` (28/28); the changed GLSL 330 composite fragment shader passed offline `glslangValidator` compilation; passed `git diff --check`; `git diff --stat` and `git status --short` reviewed.
- GPU measurements: The user screenshot reported Unified `0.794 ms` within `0.818 ms` total at Medium, with jitter period 8, one active shadowed spotlight, five dust emitters, and no skipped timing queries. This was diagnostic evidence only, not a controlled 300-frame benchmark.
- Manual verification: The user's screenshot confirmed Unified rendering and spotlight-shadow ownership were active, but exposed an unusable milky raw-atlas Froxels view and clipped Atmosphere diagnostics with no discoverable History Weight selection. Both presentation defects were corrected; post-fix visual verification remains user-owned.
- Cache invalidation behavior: Unchanged. The fixes affect renderer-local debug presentation and editor overlay layout only; no topology or cached 2D editor-state mutation was added.
- Lightmap source-hash behavior: Unchanged. Debug visualization and overlay controls remain visual/runtime-only and excluded from lightmap source hashing.
- Collision/sector lookup/physics behavior: Unchanged. The corrected Froxels view reads the existing current atmosphere target and does not affect camera, collision, sector lookup, physics, picking, or lightmap sampling.
- Legacy coexistence/removal state: Unchanged. Legacy remains available; no schema removal, map migration, or Slice 6 work was performed.
- Remaining follow-up within this plan: Rebuild and repeat the concise Composite/Froxels/History Weight acceptance checks. Slice 5 remains Partial until the user reports the temporal/disocclusion/teleport behavior acceptable.

### 2026-08-17 — Slice 5 — Partial

- Summary: Fixed a temporal resolve coordinate-space defect exposed by the user's second visual check. The pass used framebuffer coordinates to fetch current atmosphere/depth while using vertically corrected texture UVs for reprojection, pairing rays with depth from the opposite texture row and rejecting nearly all history. Current color, depth, and neighborhood lookup now use the same `fragUv` convention as reprojection. Unified diagnostic summaries now report `medium volumes`, `lights`, and `integrations` without showing irrelevant legacy haze counts. Froxels now uses a bounded false-color overlay for current opacity and radiance rather than compositing unfiltered extinction as black geometry-shaped blocks.
- Decisions/deviations folded back into plan: The reported `haze 0/0` was the legacy haze counter and did not indicate missing Unified lights; the same screenshots reported 13–15 of 13–15 clustered lights. The viewed `hub2` map has global fog explicitly disabled, so only its two localized authored fog volumes provide participating medium and most light volumes correctly produce no scattering there. Map migration/tuning remains Slice 6 and was not pulled into this fix. A real camera view-projection inversion test supplements the existing identity reprojection test.
- Files/modules materially affected: Focused temporal/composite shader code in `SectorVolumetricAtmosphereRenderer`; atmosphere summary/overlay wording; volumetric, diagnostics, and shader-policy tests; this living plan.
- Automated verification: Passed `cmake --build cmake-build-debug -j2`; passed `ctest --test-dir cmake-build-debug --output-on-failure` (28/28); the changed temporal and composite GLSL 330 fragment shaders passed offline `glslangValidator` compilation; passed `git diff --check`; `git diff --stat` and `git status --short` reviewed.
- GPU measurements: User screenshots at High reported Unified `1.160 ms` within `1.194 ms` total and Unified `1.355 ms` within `1.378 ms` total. These are consistent with the accepted Slice 2 Unified High `1.383 ms` median and far below the same-view Slice 2 Legacy High `7.030 ms` median. Legacy fog/haze timings were `0.001/0.000 ms` and `0.001/0.001 ms`, confirming the old continuous-medium passes were not executing alongside Unified.
- Manual verification: User reported the pre-fix History Weight view nearly black after thousands of valid-history frames and the raw Froxels view showing a black lower-screen block; screenshots also showed no light-shaped haze in globally fog-disabled `hub2`. The coordinate defect and debug/diagnostic presentation were corrected; post-fix temporal verification remains user-owned.
- Cache invalidation behavior: Unchanged. The fixes affect renderer-local temporal/debug sampling and diagnostic labels only; no topology or cached 2D editor-state mutation was added.
- Lightmap source-hash behavior: Unchanged. No map, lightmap input, bake artifact, or source-hash rule changed.
- Collision/sector lookup/physics behavior: Unchanged. Texture-coordinate correction and debug visualization remain isolated to atmosphere rendering.
- Legacy coexistence/removal state: Legacy remains available and measured inactive in the supplied Unified screenshots. No legacy removal, map migration, or Slice 6 work was performed.
- Remaining follow-up within this plan: Rebuild and check History Weight while stationary in Medium/High. Use `hub`, whose global fog is enabled, or temporarily enable Fog in `hub2` to assess lit-air temporal behavior; do not save that temporary `hub2` edit unless beginning Slice 6 intentionally.

### 2026-08-17 — Slice 5 — Partial

- Summary: Corrected the shared coordinate contract after the user's third visual check exposed a blue lower-screen band in Froxels, gray macroblocks in Composite, and a horizontal motion band. Integration once again derives its ray from the framebuffer froxel cell, matching medium and light injection, while scene depth uses the logical render-target `fragUv`. Temporal reprojection now uses the unjittered camera projection and screen UV; froxel jitter only moves samples inside a cell. Temporal neighborhood and bilateral composite taps now use clamped texel-center UV sampling instead of mixing render-target UVs with raw storage coordinates.
- Decisions/deviations folded back into plan: The screenshots were from `hub`, not `hub2`; the earlier map attribution was wrong. The changing `2/2` and `3/3` counts identify portal-visible local medium volumes, not the loaded map. Froxels now uses centered jitter, skips temporal resolve/history promotion, and nearest-composites the current integrated atmosphere over the scene. Entering or leaving Froxels invalidates history. History Weight continues to show accepted temporal history share, and leaving it invalidates the diagnostic weight target before normal accumulation resumes.
- Files/modules materially affected: Focused unified integration, temporal, composite, and debug-state code in `SectorVolumetricAtmosphereRenderer`; the pure reprojection helper; focused volumetric shader-policy tests and diagnostics wording; this living plan. The user's modified `assets/levels/hub/hub.json` and screenshot remain untouched by this correction.
- Automated verification: Passed `cmake --build cmake-build-debug -j2`; passed `ctest --test-dir cmake-build-debug --output-on-failure` (28/28); all five embedded GLSL 330 fragment shaders passed offline `glslangValidator`; passed `git diff --check`; `git diff --stat` and `git status --short` reviewed. CPU and source-policy tests now cover jitter-free reprojection, stable camera inversion, offscreen rejection, froxel-cell ray construction, unjittered scene-depth/reprojection UVs, texel-center atmosphere/depth sampling, and centered non-temporal Froxels behavior.
- GPU measurements: No new measurement was captured. The supplied Medium screenshots reported about `0.846–0.878 ms` for Unified, consistent with the accepted budget; the legacy fog/haze passes reported approximately zero, confirming they were not running concurrently. Slice 6 remains responsible for controlled performance capture and retirement criteria.
- Manual verification: Not performed (user-owned). The supplied pre-fix `hub` screenshots, including the user's fog density increase to `0.25`, were used as failure evidence. Slice 5 remains Partial until Composite, Froxels, History Weight, and camera-motion behavior are visually accepted.
- Cache invalidation behavior: Unchanged for topology/editor caches. Debug transitions invalidate renderer-local temporal history only; no topology or cached 2D editor-state mutation path changed.
- Lightmap source-hash behavior: Unchanged. The coordinate, temporal, reconstruction, and debug changes are visual/runtime-only and do not alter serialized lightmap inputs or source hashing.
- Collision/sector lookup/physics behavior: Unchanged. Froxel jitter, render-target UVs, and temporal camera matrices remain isolated to atmosphere rendering.
- Legacy coexistence/removal state: Legacy remains available and inactive when Unified is selected. No legacy removal, map migration, or Slice 6 work was performed.
- Remaining follow-up within this plan: The user repeats the visual check in the already modified `hub`: Composite must show global fog/lit air without gray blocks or a motion band; Froxels must show a stable blocky whole-view current result without the blue lower band; History Weight must converge while stationary and reject newly revealed pixels. Mark Slice 5 Completed only after that acceptance.

### 2026-08-17 — Slice 5 — Stabilization rollback (Partial)

- Summary: Replaced the failing active Slice 5 temporal path with a centered current-frame path. Medium and High no longer execute jitter, temporal resolve, history ping-pong, history-depth reconstruction, or volumetric spotlight shadow sampling. Composite once again uses the Slice 4 five-tap scene-depth-aware reconstruction, and Froxels nearest-samples the current integrated result. History Weight and Freeze History were removed from the Atmosphere controls. Global fog density is no longer normalized against the entire configured range; the authored density is injected directly and `maxOpacity` is enforced as an accumulated global optical-depth cap during integration.
- Decisions/deviations folded back into plan: Correct rendering is the gate before temporal/shadow work can return. Local-volume density/cap behavior is unchanged. The rendered Low/Medium/High froxel grids remain 120x68x32, 160x90x48, and 240x135x64, while CPU light/volume culling now uses four-by-four XY tiles (30x17x4 bands, 40x23x6 bands, and 60x34x8 bands). This reduces Medium cluster construction from 86,400 to 5,520 cells without lowering the integrated target resolution. Unified and Legacy remain mutually exclusive; the old passes were not running alongside Unified.
- Files/modules materially affected: Focused Unified contracts, resource layout, injection/integration/composite shaders, renderer execution, atmosphere diagnostics, and the existing Atmosphere overlay/layout; focused shader-policy, diagnostics, volumetric, and UI-layout tests; this living plan. The user's modified `assets/levels/hub/hub.json` and screenshot remain untouched.
- Automated verification: Passed `cmake --build cmake-build-debug -j2`; passed `ctest --test-dir cmake-build-debug --output-on-failure` (28/28); all four active GLSL 330 fragment shaders passed offline `glslangValidator`; passed `git diff --check`; `git diff --stat` and `git status --short` reviewed.
- GPU/CPU measurements: Not captured. Interactive testing is user-owned. The coarse cluster change specifically addresses the reported Debug Medium CPU collapse; its actual frame-rate effect still needs the user's build/run measurement.
- Manual verification: Not performed (user-owned). The pre-fix `hub` screenshots and the reported 17 FPS Debug Medium result are treated as failure evidence, not acceptance.
- Cache invalidation behavior: Unchanged. The changes affect renderer-local derived resources and diagnostics only; no topology mutation or visible cached 2D editor state changed.
- Lightmap source-hash behavior: Unchanged. Fog reconstruction, cluster resolution, runtime shadow participation, and debug controls are visual/runtime-only; no serialized lightmap input, bake result, or source-hash rule changed.
- Collision/sector lookup/physics behavior: Unchanged. Atmosphere sampling and cluster culling remain isolated from gameplay collision, sector lookup, camera motion, picking, and physics.
- Legacy coexistence/removal state: Legacy remains available for comparison and is not executed when Unified is selected. No legacy renderer/schema removal or map migration was performed.
- Remaining follow-up within this plan: Rebuild and test the already modified `hub` at Medium. Composite must show authored global fog and light-shaped scattering without gray macroblocks or a motion band; Froxels must show a stable, blocky whole-view current result without bottom-screen shapes. Compare stationary and moving Debug frame rate. Slice 5 remains Partial until those rendering and performance checks pass.
