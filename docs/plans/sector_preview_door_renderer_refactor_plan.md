...
| Phase 3: Move Door Opaque Shader And Material Ownership | Completed | 2026-07-05 | Moved door opaque shader source strings, shader/material resource lifetime, uniform locations, and door lighting debug mode state into `SectorPreviewDoorRenderer`. `SectorMeshPreview` public debug accessors now forward to the renderer and the main door draw path remains in `SectorMeshPreview`. Source changed; behavior intended unchanged. Checks passed. |
| Phase 4: Move Main Scene Door Draw Path | Completed | 2026-07-05 | Moved main-scene door ECS traversal, object-probe color upload, dynamic light/shadow receiving upload, texture/tint binding, render-state bracket, render debug text, and draw counters into `SectorPreviewDoorRenderer`. `SectorMeshPreview` still owns facade sequencing and delegates door drawing through an explicit context. Source changed; behavior intended unchanged. Checks passed. |
| Phase 5: Move Door Shadow Caster Drawing Integration | Completed | 2026-07-05 | Dynamic spotlight shadow rendering now resolves prepared door meshes through `SectorPreviewDoorRenderer` directly, using separate resolver user data from static-sector texture lookup. Source changed; behavior intended unchanged. Checks passed. |
| Phase 6: Integrate Facade Cleanup | Completed | 2026-07-05 | Removed the remaining private `SectorMeshPreview::DrawRuntimeDoors()` trampoline, narrowed the public door-renderer helper surface, and moved dynamic-shadow door context setup behind `SectorPreviewDoorRenderer::PrepareShadowRenderContext()`. Source changed; behavior intended unchanged. Checks passed. |
| Phase 7: Verification Docs And Backlog Update | Completed | 2026-07-05 | Final automated verification passed and REF-015 backlog entry marked complete. Documentation-only closeout; no source changes in this phase. Manual visual smoke was not performed. |
...
* 2026-07-05: Phase 3 completed. Moved door opaque shader source strings, shader/material load and shutdown, default material texture ownership, uniform locations, shader-location reset behavior, debug mode state, and `SectorDoorLightingDebugModeName()` into `SectorPreviewDoorRenderer`. `SectorMeshPreview` still owns public facade sequencing and the main-scene door draw path, forwarding public door debug accessors and reading renderer-owned material/uniform state during the existing draw loop. Source code changed; shader formulas, uniform names, array lookup behavior, material map slots for diffuse/roughness/occlusion, default texture reset behavior, debug-mode integer values/display names, dynamic lighting upload behavior, dynamic spotlight shadow-map resource ownership, render order, runtime door behavior, JSON/schema, lightmap source-hash behavior, topology cache behavior, collision, sector lookup, physics, and ECS lifecycle behavior were intended unchanged. Manual visual smoke was not performed. Verification passed: `cmake --build cmake-build-debug -j2`; `ctest --test-dir cmake-build-debug --output-on-failure -R "sector_runtime_object|sector_light|sector_topology|sector_editor|sector_authoring"`; `ctest --test-dir cmake-build-debug --output-on-failure`.
* 2026-07-05: Phase 4 completed. Moved the main-scene `DrawRuntimeDoors()` implementation into `SectorPreviewDoorRenderer::Draw()` and left `SectorMeshPreview::DrawRuntimeDoors()` as explicit context assembly/delegation. Door draw counters and `" | doors:"` render debug text now live in the renderer and public `SectorMeshPreview` counter accessors forward to renderer stats. Source code changed; main-draw mesh preparation fallback, visibility/enabled/render filtering, positive-dimension filtering, texture fallback, tint upload, current-transform object-probe/static-light color generation and `UpdateMeshBuffer()` upload, dynamic point/spot light receiving upload, dynamic spotlight shadow receiving upload, shadow map texture binding slots, door debug-mode uniform upload, render-state setup/restore, and static-sector/door/billboard draw order were intended unchanged. No shader formulas or uniform names, dynamic lighting/shadow selection, dynamic spotlight shadow-map resource ownership, object-probe bake/fallback policy, runtime door behavior, JSON/schema, lightmap source-hash behavior, topology cache behavior, collision, sector lookup, physics, or ECS lifecycle behavior changed. Manual visual smoke was not performed. Verification passed: `cmake --build cmake-build-debug -j2`; `ctest --test-dir cmake-build-debug --output-on-failure -R "sector_runtime_object|sector_light|sector_topology|sector_editor|sector_authoring"`; `ctest --test-dir cmake-build-debug --output-on-failure`.
* 2026-07-05: Phase 5 completed. Replaced the remaining `SectorMeshPreview` door shadow-caster mesh resolver bridge with direct `SectorPreviewDoorRenderer::ResolveDoorShadowCasterMesh` delegation from the dynamic spotlight shadow render context. Added separate door-mesh resolver user data so static-sector shadow texture lookup still uses `SectorMeshPreview` while prepared door mesh lookup uses the door renderer. Source code changed; dynamic spotlight shadow rendering orchestration, dynamic light/shadow selection, shadow-map ownership, selected shadow slots, static-sector shadow texture lookup, door shadow caster preparation timing, door mesh/width/height lookup, shadow caster model matrix behavior, seal margins, render order, shader formulas/uniforms, runtime door behavior, JSON/schema, lightmap source-hash behavior, topology cache behavior, collision, sector lookup, physics, and ECS lifecycle behavior were intended unchanged. Manual visual smoke was not performed. Verification passed: `cmake --build cmake-build-debug -j2`; `ctest --test-dir cmake-build-debug --output-on-failure -R "sector_runtime_object|sector_light|sector_topology|sector_editor|sector_authoring"`; `ctest --test-dir cmake-build-debug --output-on-failure`.
* 2026-07-05: Phase 6 completed. Removed the remaining private `SectorMeshPreview::DrawRuntimeDoors()` door facade trampoline by assembling the existing door draw context at the main-scene draw point and forwarding directly to `SectorPreviewDoorRenderer::Draw()`. Added `SectorPreviewDoorRenderer::PrepareShadowRenderContext()` so the door renderer owns preparation/clearing of prepared door shadow casters and wiring of the door mesh resolver into `SectorPreviewDynamicSpotLightShadowRenderContext`; `SectorMeshPreview` still owns the public facade method and static-sector texture resolver. Removed unused transitional door context structs and made door mesh preparation/cache lookup/shadow caster resolver access private to the door renderer. Source code changed; `SectorMeshPreview` public API, shader formulas/uniform names, dynamic lighting/shadow selection, dynamic spotlight shadow-map ownership, shadow caster seal margins, model matrix behavior, render order, runtime door behavior, JSON/schema, lightmap source-hash behavior, topology cache behavior, collision, sector lookup, physics, and ECS lifecycle behavior were intended unchanged. Manual visual smoke was not performed. Verification passed: `cmake --build cmake-build-debug -j2`; `ctest --test-dir cmake-build-debug --output-on-failure -R "sector_runtime_object|sector_light|sector_topology|sector_editor|sector_authoring"`; `ctest --test-dir cmake-build-debug --output-on-failure`.
* 2026-07-05: Phase 7 completed. Closed REF-015 verification documentation and marked REF-015 complete in `docs/plans/codebase_refactor_backlog.md` after confirming Phases 1 through 6 were completed. This closeout phase changed documentation only; previous implementation phases changed source code as logged above. Final ownership split remains: `SectorMeshPreview` is the public facade and high-level render sequencing owner, while `SectorPreviewDoorRenderer` owns door renderer GPU cache/resource lifetime, door shader/material/debug state, main door draw traversal, draw counters/debug text, prepared door shadow caster records, and door mesh resolution for dynamic spotlight shadow rendering. Behavior intended unchanged: no shader formulas or uniform names, dynamic light/shadow selection, dynamic spotlight shadow-map ownership, render order, runtime door behavior, lightmap source-hash behavior, topology cache behavior, collision, sector lookup, physics, JSON/schema, or ECS lifecycle ownership changed. Manual visual smoke was not performed, so door preview smoke remains recommended before relying on visual parity. Final verification passed: `python3 tools/plan_executor.py docs/plans/sector_preview_door_renderer_refactor_plan.md --status`; `cmake --build cmake-build-debug -j2`; `ctest --test-dir cmake-build-debug --output-on-failure -R "sector_runtime_object|sector_light|sector_topology|sector_editor|sector_authoring"`; `ctest --test-dir cmake-build-debug --output-on-failure`; `git diff --check`; `git diff --stat`; `git status --short`.
