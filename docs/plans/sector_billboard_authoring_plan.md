...
```plan-state-json id="sector-billboard-authoring"
{
  ...
  "items": [
    ...
    { "id": "phase_02", "title": "Generic Billboard Runtime Spawn And Playback", "type": "phase", "status": "Completed" },
    { "id": "phase_02a", "title": "Spawn Generic Billboard Objects Into ECS", "type": "pass", "parent": "phase_02", "status": "Completed" },
    { "id": "phase_02b", "title": "Add Single Clip Resolution And Playback State", "type": "pass", "parent": "phase_02", "status": "Completed" },
    { "id": "phase_03", "title": "Sprite Asset Scanner And Picker Modal", "type": "phase", "status": "Completed" },
    { "id": "phase_03a", "title": "Add Sprite Metadata Scanner", "type": "pass", "parent": "phase_03", "status": "Completed" },
    { "id": "phase_03b", "title": "Add Sprite Picker Modal With Atlas Preview", "type": "pass", "parent": "phase_03", "status": "Completed" },
    { "id": "phase_04", "title": "Editor Billboard Tool And Inspector", "type": "phase", "status": "Completed" },
    { "id": "phase_04a", "title": "Replace Object Tool With Billboard Placement Tool", "type": "pass", "parent": "phase_04", "status": "Completed" },
    { "id": "phase_04b", "title": "Add Billboard Inspector Fields And Aspect Ratio Editing", "type": "pass", "parent": "phase_04", "status": "Completed" },
    { "id": "phase_04c", "title": "Wire Sprite Picker And Clip Selection Into Inspector", "type": "pass", "parent": "phase_04", "status": "Completed" },
    { "id": "phase_05", "title": "Tests Documentation And Cleanup", "type": "phase", "status": "Completed" },
    { "id": "phase_05a", "title": "Strengthen Tests And Remove Goblin-Only Scaffolding", "type": "pass", "parent": "phase_05", "status": "Completed" },
    { "id": "phase_05b", "title": "Update Documentation And Close Plan", "type": "pass", "parent": "phase_05", "status": "Completed" }
  ]
}
```
...
| Phase 2: Generic Billboard Runtime Spawn And Playback | Completed | 2026-07-02 | Phase 2A and Phase 2B complete. Generic authored billboards spawn into ECS, request sprite animation assets during explicit spawn/reset/refresh, support directional clips and non-directional single clips, and use `SectorBillboardAnimator` for authored playing/paused defaults. |
| Phase 2A: Spawn Generic Billboard Objects Into ECS | Completed | 2026-07-02 | Source changed. `SpawnPlacedRuntimeObjects()` now handles authored `kind: "billboard"` objects directly instead of routing through the old runtime-object definition registry. Spawn requests `billboard.spriteAnimationPath` through the runtime-object asset scope, converts authored position through the existing placed-object position conversion, copies authored yaw, width/height world size, normalized origin, and `playing` into ECS components, and records placed-object ID to ECS entity mappings as bookkeeping only. Directional billboards get `SectorBillboardDirectionalClips` with authored front/back/left/right clip names; non-directional billboards omit that optional component and keep clip resolution deferred to Phase 2B. Legacy non-empty `definitionId` objects remain unsupported/skipped; unsupported `kind` and empty billboard sprite paths are skipped with diagnostics. Authoring refresh replaces mapped ECS entities without duplicates; full reset clears `SectorObject` entities and unloads/recreates the runtime object asset scope. Runtime diagnostics now count sprite asset request state for all billboard sprites, not only directional billboards. `SectorMeshPreview` remains renderer-only and was not changed. Topology render-cache invalidation behavior is unchanged; no topology mutation path was changed. Lightmap source-hash behavior is unchanged; runtime billboard authored data remains outside baked lightmap topology hashing. Asset requests happen during spawn/refresh/reset, not render/update loops. Verification: `cmake --build cmake-build-debug --target sector_runtime_object_tests -j2`, `ctest --test-dir cmake-build-debug -R '^sector_runtime_object$' --output-on-failure`, `cmake --build cmake-build-debug -j2`, and `ctest --test-dir cmake-build-debug --output-on-failure` passed. |
| Phase 2B: Add Single Clip Resolution And Playback State | Completed | 2026-07-02 | Source changed. Added data-only ECS component `SectorBillboardSingleClip` for non-directional authored clip names plus resolved clip index/fallback state, and reserved it with the other runtime-object components. Non-directional billboards now store `billboard.clip` in this component at spawn; directional billboards continue to use `SectorBillboardDirectionalClips` and omit the single-clip component. `UpdateSectorRuntimeObjects()` resolves single clips after the sprite animation asset is ready, writes the resolved index into `SectorBillboardSprite::clipIndex`, and falls back safely for empty or missing names using `Default` when available or clip 0 otherwise. Directional front/back/left/right resolution behavior is preserved. Size/origin remain in `SectorBillboardSprite`; authored playing/paused state remains in `SectorBillboardAnimator`, and paused animators do not advance time. Runtime diagnostics now track single-clip resolved/missing/fallback counts alongside directional clip diagnostics. Public runtime helper functions for single-clip resolution were added for tests. Asset requests still happen during spawn/reset/refresh, not update/render loops. Topology render-cache invalidation behavior is unchanged; no topology mutation path was changed. Lightmap source-hash behavior is unchanged; runtime billboard authored data remains outside baked lightmap topology hashing. Collision, sector lookup, and physics behavior are unchanged. Verification: `cmake --build cmake-build-debug --target sector_runtime_object_tests -j2`, `ctest --test-dir cmake-build-debug -R '^sector_runtime_object$' --output-on-failure`, `cmake --build cmake-build-debug -j2`, `ctest --test-dir cmake-build-debug --output-on-failure`, and `git diff --check` passed. |
| Phase 3: Sprite Asset Scanner And Picker Modal | Completed | 2026-07-02 | Phase 3A and Phase 3B complete. Sprite metadata scanning and the v1 atlas-preview picker modal are implemented; inspector wiring remains deferred to Phase 4C. |
| Phase 3A: Add Sprite Metadata Scanner | Completed | 2026-07-02 | Source changed. Added `SectorSpriteMetadata` plus `ScanAssetSpriteAsepriteJsons()` helpers for scanning `assets/sprites` recursively for `.json` files, returning normalized `assets/...` sprite JSON paths, resolved atlas image paths, and discovered clip names. Candidate validation requires parseable object-root JSON with object `frames`, object `meta`, and non-empty string `meta.image`; invalid candidates are skipped. Clip discovery uses unique `meta.frameTags[*].name` values first, then frame-name-derived clip names, then `Default` when no clip names are available. Atlas image paths are resolved relative to the sprite JSON parent and normalized back to `assets/...` when under the assets root. No textures or sprite animation assets are requested by the scanner; preview scope and modal lifecycle remain for Phase 3B. Existing texture picker/import behavior is unchanged. Topology render-cache invalidation behavior is unchanged; no topology mutation path was changed. Lightmap source-hash behavior is unchanged. Verification: `cmake --build cmake-build-debug --target sector_authoring_graph_tests -j2`, `ctest --test-dir cmake-build-debug -R '^sector_authoring_graph$' --output-on-failure`, `cmake --build cmake-build-debug -j2`, and `ctest --test-dir cmake-build-debug --output-on-failure` passed. |
| Phase 3B: Add Sprite Picker Modal With Atlas Preview | Completed | 2026-07-02 | Source changed. Added caller-owned `SpritePickerState`, `DrawSpritePickerModal()`, sprite picker scan/selection/result helpers, and editor modal lifecycle wiring. The modal scans on open or explicit Refresh, lists `assets/sprites` Aseprite JSON candidates from the Phase 3A scanner, lists discovered clip metadata for the selected sprite, and requests the selected atlas PNG through `AssetManager::RequestTexture()` with point filtering in a modal-local preview scope. The preview scope is unloaded/recreated when the selected atlas changes and unloaded when the modal closes or the editor shuts down. The picker currently returns selected sprite path, atlas path, clip names, and selected clip through helper result data; committing that selection into billboard inspector fields remains deferred to Phase 4C. Existing texture picker/import behavior is unchanged. Topology render-cache invalidation behavior is unchanged; no topology mutation path was changed. Lightmap source-hash behavior is unchanged. No manual GUI smoke was performed. Verification: `cmake --build cmake-build-debug --target sector_authoring_graph_tests -j2`, `ctest --test-dir cmake-build-debug -R '^sector_authoring_graph$' --output-on-failure`, `cmake --build cmake-build-debug -j2`, `ctest --test-dir cmake-build-debug -R '^sector_runtime_object$' --output-on-failure`, and `ctest --test-dir cmake-build-debug --output-on-failure` passed. |
| Phase 4: Editor Billboard Tool And Inspector | Completed | 2026-07-02 | Phase 4A, Phase 4B, and Phase 4C complete. The editor now has generic Billboard placement, core authored property editing, sprite picker commit, and single/directional clip assignment. |
| Phase 4A: Replace Object Tool With Billboard Placement Tool | Completed | 2026-07-02 | Source changed. The user-facing runtime-object tool label/help/status text now says `Billboard`; the click placement path creates generic authored `kind: "billboard"` objects through `AddBillboardToSector()` instead of assigning the old hardcoded object definition. Placement defaults are empty `billboard.spriteAnimationPath`, width/height `{1.0f, 1.0f}`, bottom-center origin `{0.5f, 1.0f}`, keep-aspect enabled, playing enabled, non-directional, yaw `0`, and position on the containing sector floor. The new object is selected after placement, `MarkTopologyDocumentEdited()` invalidates the 2D topology render cache, and runtime objects are refreshed; empty sprite paths remain safe runtime skips/diagnostics until Phase 4C wires sprite selection. The 2D render cache treats `kind == "billboard"` as the known marker path and no longer depends on a hardcoded definition for new placement. The old inspector definition assignment button/copy was removed without adding the Phase 4B property fields. Asset loading behavior is unchanged; no asset is requested during placement. Serialization schema and lightmap source-hash behavior are unchanged. Gameplay collision, sector lookup, and physics behavior are unchanged. No manual GUI smoke was performed. Verification: `cmake --build cmake-build-debug --target sector_authoring_graph_tests -j2`, `ctest --test-dir cmake-build-debug -R '^sector_authoring_graph$' --output-on-failure`, `cmake --build cmake-build-debug --target sector_runtime_object_tests -j2`, `ctest --test-dir cmake-build-debug -R '^sector_runtime_object$' --output-on-failure`, `cmake --build cmake-build-debug --target sector_topology_serialization_tests -j2`, `ctest --test-dir cmake-build-debug -R '^sector_topology_serialization$' --output-on-failure`, `cmake --build cmake-build-debug -j2`, and `ctest --test-dir cmake-build-debug --output-on-failure` passed. |
| Phase 4B: Add Billboard Inspector Fields And Aspect Ratio Editing | Completed | 2026-07-02 | Source changed. The selected billboard inspector now exposes Object ID, type/sprite status, Position X/Y/Z, Yaw, Width, Height, Keep aspect ratio, Origin X/Y, Directional, Playing, and Delete. Width/height edits are clamped to positive world-unit values; origin edits are clamped to normalized `0..1`; directional and playing checkboxes write authored billboard data. Keep-aspect editing derives aspect from the selected billboard's ready sprite animation metadata when available, preferring the chosen/front clip's first frame source size and falling back to source rectangle dimensions; when metadata is not available, the paired dimension is left unchanged and the inspector shows an aspect-unavailable status. Mutations use `MutateSelectedRuntimeObject()`, so authored data is marked dirty, the 2D topology render cache is invalidated through `MarkTopologyDocumentEdited()`, and ECS preview runtime objects are refreshed. Sprite picker and clip selection commit behavior remains deferred to Phase 4C. Serialization schema, asset request lifecycle, lightmap source-hash behavior, gameplay collision, sector lookup, and physics behavior are unchanged. No manual GUI smoke was performed. Verification: `cmake --build cmake-build-debug --target sector_authoring_graph_tests -j2`, `cmake --build cmake-build-debug -j2`, `ctest --test-dir cmake-build-debug -R '^sector_authoring_graph$' --output-on-failure`, `ctest --test-dir cmake-build-debug -R '^sector_runtime_object$' --output-on-failure`, `ctest --test-dir cmake-build-debug -R '^sector_topology_serialization$' --output-on-failure`, `ctest --test-dir cmake-build-debug --output-on-failure`, and `git diff --check` passed; `git diff --stat` and `git status --short` reviewed. |
| Phase 4C: Wire Sprite Picker And Clip Selection Into Inspector | Completed | 2026-07-02 | Source changed. The selected billboard inspector now opens the sprite picker for the current billboard and commits picker results into authored `billboard.spriteAnimationPath` plus either the single `clip` field or the chosen directional `frontClip`/`backClip`/`leftClip`/`rightClip` field. The modal preselects the current sprite/clip when available, keeps scan/preview behavior from Phase 3B, and reuses the clip list modal pattern instead of adding a dropdown inside the inspector scroll area. Picker commit fills empty/invalid clip fields from selected metadata, preferring Front/Back/Left/Right directional defaults when those clips exist and otherwise falling back to the selected/first available clip. Mutations use `MutateSelectedRuntimeObject()`, so authored data is marked dirty, the 2D topology render cache is invalidated through `MarkTopologyDocumentEdited()`, and ECS preview runtime objects are refreshed. Aspect-ratio dimensions are not changed directly on picker commit unless the selected sprite asset is already available through the existing runtime metadata path; existing keep-aspect numeric editing remains unchanged. Serialization schema, asset request lifecycle, lightmap source-hash behavior, gameplay collision, sector lookup, and physics behavior are unchanged. No manual GUI smoke was performed. Verification: `cmake --build cmake-build-debug --target sector_authoring_graph_tests -j2`, `ctest --test-dir cmake-build-debug -R '^sector_authoring_graph$' --output-on-failure`, `cmake --build cmake-build-debug -j2`, `ctest --test-dir cmake-build-debug -R '^sector_runtime_object$' --output-on-failure`, `ctest --test-dir cmake-build-debug -R '^sector_topology_serialization$' --output-on-failure`, and `ctest --test-dir cmake-build-debug --output-on-failure` passed. |
| Phase 5: Tests Documentation And Cleanup | Completed | 2026-07-02 | Phase 5A and Phase 5B complete. Tests, cleanup, documentation, and plan closure are complete for the generic Billboard authoring feature. |
| Phase 5A: Strengthen Tests And Remove Goblin-Only Scaffolding | Completed | 2026-07-02 | Source changed. Removed the obsolete hardcoded runtime-object definition registry/API (`SectorRuntimeObjectDefinition`, `SectorRuntimeObjectBillboardDefinition`, `GetSectorRuntimeObjectDefinitions()`, and `FindSectorRuntimeObjectDefinition()`), so `definitionId: "goblin"` has no supported authored runtime path. Runtime-object tests no longer expect a built-in goblin definition; the checked-in `assets/sprites/goblin.*` files are covered only as ordinary billboard sprite fixture data, and directional clip-name tests now exercise generic authored billboard data. Existing legacy `definitionId: "goblin"` unsupported/skipped behavior remains explicitly tested. User-facing editor text remains Billboard-focused from Phase 4, and source search found no F5 goblin/debug spawn symbols. Runtime behavior for generic billboards, asset request lifecycle, ECS ownership, `SectorMeshPreview` renderer-only behavior, serialization behavior, topology cache invalidation behavior, lightmap source-hash behavior, collision, sector lookup, and physics are unchanged. No documentation update beyond this active plan pass; Phase 5B remains pending. No manual GUI smoke was performed. Verification: `cmake --build cmake-build-debug --target sector_runtime_object_tests -j2`, `ctest --test-dir cmake-build-debug -R '^sector_runtime_object$' --output-on-failure`, `cmake --build cmake-build-debug -j2`, and `ctest --test-dir cmake-build-debug --output-on-failure` passed; `git diff --check`, `git diff --stat`, and `git status --short` reviewed. |
| Phase 5B: Update Documentation And Close Plan | Completed | 2026-07-02 | Docs-only/source-code unchanged pass. Updated `docs/sector_editor.md` to document the Billboard 2D tool, bottom-center default anchor, width/height world units, keep-aspect behavior, sprite picker scan of `assets/sprites`, single versus directional clips, playing/paused authoring, supported Aseprite timing/modes, cutout-only rendering, and deferred NPC/collision/scripting/3D-model/transparent-sprite behavior. The docs now state that `goblin` is no longer a special editor object type and that checked-in goblin sprite files are ordinary selectable sprite assets only. This active plan was closed by marking Phase 5 and Phase 5B completed. No manual GUI smoke was performed. Runtime behavior, serialization behavior, asset loading behavior, ECS ownership, topology cache invalidation behavior, lightmap source-hash behavior, collision, sector lookup, and physics are unchanged. Verification: `cmake --build cmake-build-debug -j2`, `ctest --test-dir cmake-build-debug --output-on-failure`, and `git diff --check` passed; `git diff --stat` and `git status --short` reviewed. |
