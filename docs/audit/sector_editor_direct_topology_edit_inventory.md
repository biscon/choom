...
| `DrawTopologySideDefMaterialInspector` | read | selected linedef/sidedef, portal flag, material display state | OK read-only consumer | Continue routing material edits through `MaterialEditingService` | Blocks Player display reads derived topology. |
...
| `SectorEditorMaterialEditingService::ApplyAuthoringSideMaterialEdit` | write | resolves derived sidedef to `SectorAuthoringLineSide`, then mutates wall/lower/upper/middle material fields directly | Fixed in REF-071; renamed in REF-072 | None for this path | Missing authoring side overrides may be seeded from current derived display values by existing authoring mutation helpers; the final edit is assigned directly to authoring data. |
| `SectorEditorMaterialEditingService::ApplyAuthoringFaceAnchorMaterialEdit` | write | resolves floor/ceiling derived target to `SectorAuthoringFaceAnchor`, then mutates floor/ceiling texture, UV, or decal fields directly | Fixed in REF-071; renamed in REF-072 | None for this path | No topology material action is run for authoring flat edits. |
| `SectorEditorMaterialPickerRouting` authoring routes | write | material picker selections resolve to authoring side/face targets and assign selected texture directly | Fixed in REF-071 | None for material routes | RuntimeDoor, MapSky, sprite, and add-map texture picker behavior remains outside material routing. |
...
| `DrawTopologySectorInspector` | write | authoring face-anchor height, sky, ambient, material, decal, and sector/default-wall UV fields | authoring-owned edit | None for existing fields; add/follow up any future field missing from authoring graph | Material operations call `MaterialEditingService` directly after REF-072; invalid no-authoring edit state fails with an authoring-required status and does not mutate `SectorTopologySector`. |
...
| `OpenMaterialPickerForDerivedSector` / `OpenMaterialPickerForDerivedSideDef` | write | authoring face-anchor/side material targets selected through derived topology IDs | authoring-owned edit | None for material routing | Editor-facing names indicate derived target selection, not topology-map editing. |
...
- REF-071 removed the remaining known normal editor material topology scratch/writeback routes; REF-072 removed stale helper names, wrapper paths, and obsolete scratch/writeback tests. No remaining normal editor material scratch/writeback route is currently listed.
- Remaining material source-of-truth debt: none currently known. Future material editing must continue to write authoring data directly or fail clearly.
