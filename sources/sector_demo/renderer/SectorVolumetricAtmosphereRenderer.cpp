...
int listIndex(sampler2D lists,ivec2 cell,int band,int slot,ivec2 grid){ivec2 clusters=ivec2(clusterGridSize.xy+vec2(0.5));ivec2 clusterCell=clamp(ivec2(cell.x*clusters.x/grid.x,cell.y*clusters.y/grid.y),ivec2(0),clusters-ivec2(1));int word=slot/4;int channel=slot-word*4;vec4 encodedIndices=texelFetch(lists,ivec2(clusterCell.x*4+word,band*clusters.y+clusterCell.y),0);return int(floor(encodedIndices[channel]*255.0+0.5));}
void main(){
 ...
 if(fogEnabled!=0&&viewDepth>=fogStartDistance&&fogDensity>0.0){float above=max(p.y-fogReferenceHeight,0.0);float sigma=fogDensity*exp(-above*fogHeightFalloff);extinction+=sigma;weightedTint+=fogColor*sigma;}
...
int listIndex(ivec2 cell,int band,int slot,ivec2 grid){ivec2 clusters=ivec2(clusterGridSize.xy+vec2(0.5));ivec2 clusterCell=clamp(ivec2(cell.x*clusters.x/grid.x,cell.y*clusters.y/grid.y),ivec2(0),clusters-ivec2(1));int word=slot/4;int channel=slot-word*4;vec4 encodedIndices=texelFetch(lightLists,ivec2(clusterCell.x*4+word,band*clusters.y+clusterCell.y),0);return int(floor(encodedIndices[channel]*255.0+0.5));}
void main(){
...
void main(){
 ...
 float transmittance=1.0;float globalOpticalDepth=0.0;float globalCap=capDepth(fogMaximumOpacity);vec3 radiance=vec3(0);for(int z=0;z<64;++z){if(z>=sliceCount||sliceDepths[z]>=sceneForward||transmittance<=0.0001)break;float segmentEnd=min(sliceDepths[z+1],sceneForward);float lengthWorld=max(segmentEnd-sliceDepths[z],0.0)/cosineForward;if(lengthWorld<=0.0)continue;ivec2 atlas=ivec2((z%tileColumns)*grid.x+cell.x,(z/tileColumns)*grid.y+cell.y);vec4 lighting=texelFetch(lightingAtlas,atlas,0);float sigma=max(lighting.a,0.0);if(sigma<=0.0)continue;float viewDepth=sqrt(sliceDepths[z]*segmentEnd);vec3 p=cameraPosition+ray*viewDepth;float globalSigma=0.0;if(fogEnabled!=0&&viewDepth>=fogStartDistance&&fogDensity>0.0){globalSigma=fogDensity*exp(-max(p.y-fogReferenceHeight,0.0)*fogHeightFalloff);}globalSigma=min(globalSigma,sigma);float localSigma=max(sigma-globalSigma,0.0);float acceptedGlobalDepth=min(globalSigma*lengthWorld,max(globalCap-globalOpticalDepth,0.0));globalOpticalDepth+=acceptedGlobalDepth;float opticalDepth=localSigma*lengthWorld+acceptedGlobalDepth;if(opticalDepth<=0.0)continue;float stepTransmittance=exp(-opticalDepth);float stepOpacity=1.0-stepTransmittance;vec3 source=max(lighting.rgb,vec3(0))/max(sigma,0.000001);radiance+=transmittance*stepOpacity*source;transmittance*=stepTransmittance;}
...
void main(){
 ivec2 size=textureSize(currentAtmosphere,0);ivec2 cell=clamp(ivec2(fragUv*vec2(size)),ivec2(0),size-ivec2(1));vec2 currentUv=(vec2(cell)+vec2(0.5))/vec2(size);
 vec4 current=sanitizeAtmosphere(texture(currentAtmosphere,currentUv));float depth=texture(currentDepth,currentUv).r;gl_FragDepth=depth;
 vec4 minimumValue=vec4(65504.0);vec4 maximumValue=vec4(0);for(int y=-1;y<=1;++y)for(int x=-1;x<=1;++x){ivec2 neighbor=clamp(cell+ivec2(x,y),ivec2(0),size-ivec2(1));vec2 neighborUv=(vec2(neighbor)+vec2(0.5))/vec2(size);vec4 value=sanitizeAtmosphere(texture(currentAtmosphere,neighborUv));minimumValue=min(minimumValue,value);maximumValue=max(maximumValue,value);}
 float historyWeight=0.0;vec4 resolved=current;
 // The camera projection is not jittered. Froxel jitter must never move
 // screen-space reprojection or render-target texture coordinates.
 if(historyValid!=0&&!isnan(depth)&&!isinf(depth)){vec4 clip=vec4(fragUv*2.0-1.0,depth*2.0-1.0,1.0);vec4 world=inverseCurrentViewProjection*clip;if(abs(world.w)>0.000001){world/=world.w;vec4 previousClip=previousViewProjection*world;if(previousClip.w>0.000001){vec3 previousNdc=previousClip.xyz/previousClip.w;vec2 historyUv=previousNdc.xy*0.5+0.5;float expectedDepth=previousNdc.z*0.5+0.5;if(all(greaterThanEqual(historyUv,vec2(0)))&&all(lessThanEqual(historyUv,vec2(1)))&&expectedDepth>=0.0&&expectedDepth<=1.0){float previousDepth=texture(historyDepth,historyUv).r;float expectedWorld=linearDepth(expectedDepth);float sampledWorld=linearDepth(previousDepth);float tolerance=max(0.10,expectedWorld*0.02);if(abs(expectedWorld-sampledWorld)<=tolerance){vec4 history=sanitizeAtmosphere(texture(historyAtmosphere,historyUv));history=clamp(history,minimumValue,maximumValue);float currentLuma=dot(current.rgb,vec3(0.2126,0.7152,0.0722));float delta=max(length(history.rgb-current.rgb)/max(currentLuma+0.05,0.05),abs(history.a-current.a)*2.0);float currentWeight=mix(baseCurrentWeight,responsiveCurrentWeight,clamp(delta,0.0,1.0));historyWeight=1.0-currentWeight;resolved=mix(history,current,currentWeight);}}}}}
 if(outputHistoryWeight!=0){finalColor=vec4(vec3(historyWeight),1.0);return;}finalColor=sanitizeAtmosphere(resolved);
}
)";
...
vec3 finiteRadiance(vec3 v){return vec3(finiteHalf(v.r),finiteHalf(v.g),finiteHalf(v.b));}
void main(){
 vec4 scene=texture(sceneColor,fragUv);float alpha=(isnan(scene.a)||isinf(scene.a))?1.0:clamp(scene.a,0.0,1.0);vec4 atmosphere=vec4(0);
 if(debugView==1){ivec2 size=textureSize(atmosphereTexture,0);ivec2 cell=clamp(ivec2(fragUv*vec2(size)),ivec2(0),size-ivec2(1));atmosphere=texelFetch(atmosphereTexture,cell,0);}
 else{float centerDepth=texture(sceneDepth,fragUv).r;float totalWeight=0.0;for(int y=-1;y<=1;++y)for(int x=-1;x<=1;++x){if(x!=0&&y!=0)continue;vec2 uv=clamp(fragUv+vec2(x,y)*atmosphereTexelSize,vec2(0),vec2(1));float sampleDepth=texture(sceneDepth,uv).r;float difference=(isnan(centerDepth)||isinf(centerDepth)||isnan(sampleDepth)||isinf(sampleDepth))?0.0:abs(sampleDepth-centerDepth);float weight=exp(-difference*600.0);atmosphere+=texture(atmosphereTexture,uv)*weight;totalWeight+=weight;}atmosphere/=max(totalWeight,0.0001);}
 float opacity=(isnan(atmosphere.a)||isinf(atmosphere.a))?0.0:clamp(atmosphere.a,0.0,1.0);vec3 composed=finiteRadiance(scene.rgb)*(1.0-opacity)+finiteRadiance(atmosphere.rgb);finalColor=vec4(finiteRadiance(composed),alpha);
}
)";
...
[[maybe_unused]] void HashBytes(std::uint64_t& hash, const void* data, std::size_t size)
{
...
#define LOC(field, name) locations.field = GetShaderLocation(value, name)
    ...
    LOC(clusterGridSize, "clusterGridSize");
...
#define ILOC(field, name) integrationLocations.field = GetShaderLocation(integrationShader, name)
    ...
    ILOC(fogEnabled, "fogEnabled");
    ILOC(fogStartDistance, "fogStartDistance");
    ILOC(fogDensity, "fogDensity");
    ILOC(fogMaximumOpacity, "fogMaximumOpacity");
    ILOC(fogReferenceHeight, "fogReferenceHeight");
    ILOC(fogHeightFalloff, "fogHeightFalloff");
#undef ILOC
...
#undef TLOC
    ...
    compositeLocations.atmosphereTexelSize = GetShaderLocation(
            compositeShader, "atmosphereTexelSize");
        ...
        const CommonLocations& locations) const
{
    ...
    const Vector3 clusterGridSize{
            static_cast<float>(resourceLayout.clusterGrid.x),
            static_cast<float>(resourceLayout.clusterGrid.y),
            static_cast<float>(resourceLayout.clusterGrid.z)};
...
#define SET(field, data, type) SetShaderValue(value, locations.field, &data, type)
    ...
    SET(clusterGridSize, clusterGridSize, SHADER_UNIFORM_VEC3);
...
#define ISET(field, data, type) SetShaderValue(integrationShader, integrationLocations.field, &data, type)
    ISET(fogEnabled, fogEnabled, SHADER_UNIFORM_INT);
    ISET(fogStartDistance, fogSettings.startDistanceWorld, SHADER_UNIFORM_FLOAT);
    ISET(fogDensity, fogSettings.density, SHADER_UNIFORM_FLOAT);
    ISET(fogMaximumOpacity, fogSettings.maxOpacity, SHADER_UNIFORM_FLOAT);
    ISET(fogReferenceHeight, fogSettings.referenceHeightWorld, SHADER_UNIFORM_FLOAT);
    ISET(fogHeightFalloff, fogSettings.heightFalloff, SHADER_UNIFORM_FLOAT);
#undef ISET
    rlDisableColorBlend();
    DrawTexturePro(sceneTarget.texture, SourceRect(sceneTarget.texture),
    ...
    const bool runTemporal = temporalPolicy.enabled
            && debugView != SectorVolumetricDebugView::Froxels;
    if (runTemporal) {
    ...
    if (debugView == SectorVolumetricDebugView::Froxels) {
        // Show the current integrated froxel columns before temporal resolve and
        // bilateral reconstruction. Nearest sampling makes the low-resolution
        // cells visible without replacing the scene with an unrelated atlas tile.
        atmosphereTarget = &integratedTarget;
    }
    ...
    const Vector2 atmosphereTexelSize{
            1.0f / static_cast<float>(atmosphereTarget->native.texture.width),
            1.0f / static_cast<float>(atmosphereTarget->native.texture.height)};
    const int debugMode = static_cast<int>(debugView);
    SetShaderValue(compositeShader, compositeLocations.atmosphereTexelSize,
            &atmosphereTexelSize, SHADER_UNIFORM_VEC2);
    ...
    rlEnableColorBlend();
    EndTextureMode();
    if (runTemporal && !historyFrozen && !outputHistoryWeight) {
        ...
        SectorVolumetricDebugView view)
{
    ...
    const bool changingFroxelMode =
            debugView == SectorVolumetricDebugView::Froxels
            || view == SectorVolumetricDebugView::Froxels;
    const bool leavingHistoryWeight =
            debugView == SectorVolumetricDebugView::HistoryWeight;
    debugView = view;
    if (changingFroxelMode || leavingHistoryWeight) {
