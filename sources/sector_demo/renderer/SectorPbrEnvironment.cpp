        ...
        SectorPbrEnvironment& outEnvironment)
{
    ...
    for (const auto& definition : map.compiledReflectionProbes) {
        if (!definition.enabled) continue;
        SectorPbrEnvironment::LocalProbe probe;
        probe.definition = definition;
        probe.ready = false;
        probe.mipCount = SectorReflectionProbeMipCount(definition.resolution);
        const std::string key = "runtime-reflection-" + std::to_string(definition.sourceAuthoringProbeId);
    ...
    BuildRuntimeSectorVisibilityGraph(map, graph);
    outEnvironment.portals = std::move(graph.portals);
    Image source{};
